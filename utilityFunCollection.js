...
utilityFunObj.createForcastObj = weatherObjList => {
  ...
  weatherObjList.data.forEach(objItem => {
    forcastObjList.push(new Forcast(objItem));
  });
...
utilityFunObj.createMovieObj = moviesObjList => {
  ...
  moviesObjList.results.forEach(movieObj =>
    newMoviesObjList.push(new Movies(movieObj))
  );
...
utilityFunObj.getFullHotelsData = async hotelData => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    hotelId =
      hotelId > 999999999
        ? utilityFunObj.getRandomHotelId(0, 999999999)
        : hotelId;
    const options = {
      method: 'GET',
    ...
    const resData = await axios.request(options);
    hotelData[i].modalImgUrl = resData.data[0].mainUrl.concat(
      '?impolicy=fcrop&w=750&h=450&q=high'
    );
...
utilityFunObj.createCovidObj = covidData => {
  return new Covid(covidData[0]);
};

utilityFunObj.getCountryName = cityName => {
  const targetContryObj = countryDataList.find(
    countryItem =>
      countryItem.capital.toLocaleLowerCase() === cityName.toLocaleLowerCase()
  );
  return targetContryObj ? targetContryObj.name : 'N/A';
};

utilityFunObj.createAndActivityList = activitiesList => {
  const activitiesObjList = [];
  activitiesList.forEach(activity =>
    activitiesObjList.push(new Activity(activity))
  );
...
utilityFunObj.createRestaurantObj = restaurantItem => {
  ...
  restaurantItem.forEach(objItem =>
    restaurantObjList.push(new restaurant(objItem))
  );
