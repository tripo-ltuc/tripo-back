...
collectionObj.locatioIQHandler = (req, res) => {
  ...
  axios
    .get(`${process.env.LOCATIONIQ_URL}`, {
      params: { q: cityName, format: 'json' },
    })
    .then(result => {
      res
        .status(200)
        .send(utilityFunCollection.creatLocationObj(result.data[0]));
    })
    .catch(err => {
...
collectionObj.weatherForcastHandler = (req, res) => {
  ...
  axios
    .get(`${process.env.WATHER_API_URL}`, {
      params: {
        city: city,
        lat: lat,
        lon: lon,
...
collectionObj.resturantHandler = (req, res) => {
  ...
  superagent
    .get(`${process.env.YELP_RESTURANTS_URL}?location=${cityName}`)
    .set('Authorization', `Bearer ${apiKey}`)
    .then(result =>
      res.send(utilityFunCollection.createRestaurantObj(result.body.businesses))
    )
    .catch(err => {
      console.log(err);
    });
...
collectionObj.activitiesHandler = (req, res) => {
  ...
  amadeus.shopping.activities
    ...
    .then(result =>
      res.send(utilityFunCollection.createAndActivityList(result.data))
    )
    .catch(err => {
      console.log(err);
    });
